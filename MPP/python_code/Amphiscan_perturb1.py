...
#Prepare to calculate the tilt
v11 = HelixTools().calculate_screw_axis(rePose)
inputdegree = HelixTools().calc_angle(v11, 'z')
...
for dist in zRange:
    ...
    # Calculate best pose tilt
    v12 = HelixTools().calculate_screw_axis(best_pose)
    bestdegree = HelixTools().calc_angle(v12, 'z')
    besttilt = bestdegree - inputdegree
    ...
    if dist > scan_stop_region:
        ...
        #record the tilt
        with open("results/{}_{}/txt/tilt.txt".format(protein_tag,multiple_tag, protein.split(".",1)[0]), "w") as tiltdump:
            tiltdump.write("The tilt for the best {} is {}\n".format(protein_tag+"_"+multiple_tag, besttilt))

        #record the real depth
        with open("results/{}_{}/txt/depth.txt".format(protein_tag,multiple_tag, protein.split(".",1)[0]), "w") as depthdump:
            if best_z > 0:
                depthdump.write("The real depth of the best {} is: {}".format(protein_tag+"_"+multiple_tag, thickness_from_file - best_z))
            else:
                depthdump.write("The real depth of the best {} is: {}".format(protein_tag+"_"+multiple_tag, thickness_from_file + best_z))
