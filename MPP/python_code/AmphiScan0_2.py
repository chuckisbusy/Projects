...
for dist in zRange:
    ...
    rotator = pyrosetta.rosetta.protocols.rigid.WholeBodyRotationMover(Vector(rand.randint(-1,1), rand.randint(-1,1), rand.randint(-1,1)),
            pyrosetta.rosetta.core.pose.center_of_mass(rePose2, range_start, range_end),
            rand.randint(1,90))
    rotator.reinitialize_for_new_input()
    rotator.apply(rePose2)

    #use a 3-step procedure to zero-in to the best pose. The rotation perturbation amount goes down with each step by half 5/3/1
    for i in range(1,numPert+1):
        rePose3 = rePose2.clone()
        rotator = pyrosetta.rosetta.protocols.rigid.WholeBodyRotationMover(Vector(rand.randint(-1,1), rand.randint(-1,1), rand.randint(-1,1)),pyrosetta.rosetta.core.pose.center_of_mass(rePose3, range_start, range_end), 20)
        rotator.reinitialize_for_new_input()
        ...
        rotator = pyrosetta.rosetta.protocols.rigid.WholeBodyRotationMover(Vector(rand.randint(-1,1), rand.randint(-1,1), rand.randint(-1,1)),pyrosetta.rosetta.core.pose.center_of_mass(rePose3, range_start, range_end), 10)
        rotator.reinitialize_for_new_input()
        rotator.apply(rePose3)
        ...
        rotator = pyrosetta.rosetta.protocols.rigid.WholeBodyRotationMover(Vector(rand.randint(-1,1), rand.randint(-1,1), rand.randint(-1,1)),pyrosetta.rosetta.core.pose.center_of_mass(rePose3, range_start, range_end), 1)
        rotator.reinitialize_for_new_input()
        rotator.apply(rePose3)
