...
thickness_from_file = args.thickness
#scan parameters  
z_final = 25
starting_z = -26
...
# randomize1 = pyrosetta.rosetta.protocols.rigid.RigidBodyRandomizeMover(rePose, 1, rigid_moves.partner_upstream)
# randomize1.apply(rePose)

# Random move with RotationMover
# rotator = pyrosetta.rosetta.protocols.rigid.WholeBodyRotationMover(Vector(rand.randint(0,1), rand.randint(0,1), rand.randint(0,1)),
#    pyrosetta.rosetta.core.pose.center_of_mass(rePose, range_start, range_end),
#    rand.randint(1,90))
# rotator.reinitialize_for_new_input()
# rotator.apply(rePose)

# Random move with SpinMover
spin_mover = pyrosetta.rosetta.protocols.rigid.RigidBodySpinMover(1)
spin_mover.rot_center(pyrosetta.rosetta.core.pose.center_of_mass(rePose, range_start, range_end))
spin_mover.spin_mag(60.0)
spin_mover.spin_axis(Vector(0,1,0))
spin_mover.apply(rePose)
...
for dist in zRange:
    ...
    for i in range(1,numPert+1):
        ...
        #perturbations with a 3 degree rotation
        rotator = pyrosetta.rosetta.protocols.rigid.WholeBodyRotationMover(Vector(rand.randint(-1,1), rand.randint(-1,1), rand.randint(-1,1)),
            pyrosetta.rosetta.core.pose.center_of_mass(rePose3, range_start, range_end), 
            5)
        rotator.reinitialize_for_new_input()
